...
class ML(method.Method):
	...
	def train(self, truth, features):
		...
		# We want the truth labels to be 1 or -1:
		gtruth = copy.copy(truth)
		gtruth[gtruth == 0] = -1
				
		# And we call the tool's train method:
		self.tool.train(features=features, labels=gtruth.reshape((len(truth), 1)))
	...
	def predict_proba(self, catalog, return_uncer=False):
		...
		preddata, preduncer = self.tool.predict(features=catalog)
		
		# Returning something between 0 and 1
		preddata += 1.
		preddata /= 2.
