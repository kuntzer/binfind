...
for iix, (crit_angsep, crit_contrast) in enumerate(criteria):
	...
	fids = u.get_id_catalogs_inverted(crit_angsep, crit_contrast)
	...
	e1_deform = features[:,:4]
	e2_deform = features[:,4:8]
	r2_deform = features[:,8:]
	
	obj_to_save = [crits_angsep, crits_contrast, e1_deform, e2_deform, r2_deform]

	if not os.path.exists(outdir):
		binfind.utils.mkdir(outdir)
	binfind.utils.writepickle(obj_to_save, os.path.join(outdir, "dang_stars_{:d}_{:1.1f}.pkl".format(int(crit_angsep*1e3), crit_contrast)))
